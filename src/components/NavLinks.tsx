        ...
        {links.map((link) => (
          <li key={link.route} className="w-full">
            <button
              ...
              className={`w-full text-left block px-4 py-3 transition-colors focus:outline-none ${
                activeId === link.route
    ...
    <div className={`flex ${className}`}> 
      {links.map((link) => (
        <button
          key={link.route}
          onClick={() => onLinkClick(link.route)}
          className={`text-foreground transition-colors px-2 py-1 rounded-md focus:outline-none ${
            activeId === link.route
